  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        if (isAdminUser) {
          setIsAdmin(true);
          const { data: adminData } = await supabase.from('user_data').select('dogs').eq('user_id', user.id).single();
          const adminDogs = adminData?.dogs ?? [];
          const { data: allData } = await supabase.from('user_data').select('dogs');
          if (allData) {
            const dogsWithRecipe = allData.flatMap(u => (u.dogs ?? []).filter(d => d.recipe?.length > 0 && !adminDogs.find(ad => ad.id === d.id)));
            const allDogs = [...adminDogs, ...dogsWithRecipe];
            setAllDogsWithRecipes(allDogs);
            if (allDogs.length > 0) setAdminSelectedDogId(allDogs[0].id);
          }
        }
