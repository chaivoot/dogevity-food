...
export default function PageDashboard({ dog, dogs, activeDogId, setActiveDogId, isAdmin, allDogsWithRecipes, adminSelectedDogId, setAdminSelectedDogId }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* Dog carousel */}
      {(isAdmin && allDogsWithRecipes.length > 0) || (!isAdmin && dogs.length > 1) ? (
        <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 8, scrollBehavior: 'smooth' }}>
          {(isAdmin ? allDogsWithRecipes : dogs).map(d => (
            <div
              key={d.id}
              onClick={() => isAdmin ? setAdminSelectedDogId(d.id) : setActiveDogId(d.id)}
              style={{
                flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
                cursor: 'pointer', padding: '8px 12px', borderRadius: 12,
                background: d.id === (isAdmin ? adminSelectedDogId : activeDogId) ? 'var(--teal-xlight)' : 'transparent',
                border: `2px solid ${d.id === (isAdmin ? adminSelectedDogId : activeDogId) ? 'var(--teal)' : 'var(--border)'}`,
              }}
            >
              ...
              <div style={{ fontSize: 10, fontWeight: d.id === (isAdmin ? adminSelectedDogId : activeDogId) ? 700 : 400, color: d.id === (isAdmin ? adminSelectedDogId : activeDogId) ? 'var(--teal)' : 'var(--text)', maxWidth: 60, textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {d.name || '(ไม่มีชื่อ)'}
              </div>
            </div>
          ))}
