...
export default function PageWeight({ dog, updateDog }) {
  const [newWeight, setNewWeight] = useState('');
  const [newDate, setNewDate] = useState(new Date().toISOString().split('T')[0]);

  const weights = dog.weights ?? [];
  const targetWeight = dog.targetWeight || null;

  const addWeight = () => {
    if (!newWeight) return;
    const updated = [...weights, { date: newDate, weight: +newWeight }]
      .sort((a, b) => a.date.localeCompare(b.date));
    updateDog({ weights: updated, weight: +newWeight });
    setNewWeight('');
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="wcard">
        ...
        <LineChart data={weights} color="var(--teal)" targetWeight={targetWeight} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 10, fontSize: 12, color: 'var(--text-light)' }}>
          ...
          {targetWeight && (
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <svg width="20" height="4"><line x1="0" y1="2" x2="20" y2="2" stroke="var(--gold)" strokeWidth="1.5" strokeDasharray="4,2" /></svg>เป้าหมาย {targetWeight} กก.
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid-2">
        ...
        <div className="wcard">
          ...
          {sorted.length === 0 ? (
            <div style={{ color: 'var(--text-light)', fontSize: 13, padding: '12px 0' }}>ยังไม่มีข้อมูลน้ำหนัก</div>
          ) : (
